...
class ApiHandler:
  ...
  def login(self, authorization):
    try:
      ...
      oauth_response = self.http.send_request("POST", "/oauth/token", authorization)
      ...
      self.http.add_header("Authorization", oauth_response["token_type"] + " " + self.access_token)
    except Exception as e:
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to connect or login - check http_handler")
      return False
  ...
  def close(self):
    try:
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to close connection or kill sockets - check http_handler")
      return False
  ...
  def get_graphs(self):
    try:
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to get or set graphs.")
      return False
  ...
  def crawl(self, graph_id, access_token):
    try:
      data = self.http.send_request("POST", "/api/v1/crawl_sessions/"+str(graph_id), None)
      crawl_session = {
        'data': data,
        'graph_id': graph_id,
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to crawl - Check that access token hasn't already been used on another graph.")
      return False
  ...
  def get_people(self, crawl_session, people_list):
    try:
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to get people - check crawl_session and ensure you are passing at least an empty empty along with the crawl_session.")
      return False
  ...
  def get_random_people(self, people_list):
    try:
      self.sort_people(people_list)
      random_selection = self.http.send_request("GET", "/api/v1/graphs/random_people", None)
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to get random people - Check that you've initated a crawl as that's a prerequisite ")
      return False
    ...
    try:
      ...
      if people_list == []:
        for i in range(len(self.crawled_people)):
          friend_list = self.http.send_request("GET", "/api/v1/friends/"+str(self.crawled_people[i]), None)
          local_friend_network.append(friend_list)
          self.friend_network.append(friend_list)
        return local_friend_network
      elif people_list != []:
        self.sort_people(people_list)
        for i in range(len(people_list)):
          friend_list = self.http.send_request("GET", "/api/v1/friends/"+str(people_list[i]), None)
          local_friend_network.append(friend_list)
          self.friend_network.append(friend_list)
        return local_friend_network
    except Exception as e:
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to get list of friends - ensure you are passing at least an empty array..")
      return False
  ...
  def sort_people(self, people_list):
    try:
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to sort people correctly.")
      return False
    
  """ Gets a challenge if you have one. If none will return False. """
  def get_challenge(self):
    try:
      challenge = self.http.send_request("GET", "/api/v1/challenges/", None)
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to get challegne - make sure you have one!")
      return False
  ...
  def post_challenge(self, solution):
    try:
      response = self.http.send_request("POST", "/api/v1/challenges/", solution)
      return response
    except Exception as e:
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to post challenge.")
      return False
    ...
    try:
      if people_list == []:
        ...
        for i in range(len(self.crawled_people)):
          ...
          beet = self.http.send_request("GET", "/api/v1/beets/"+str(self.crawled_people[i]), None)
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to get flags - ensure you're passing at least an empty list.")
      return False
  ...
  * Recieving data as Python Dict - in memory object vs JSON - string representation of an object
  ...
  def jsonify(self, dict_data):
    try:
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to jsonify - check you've imported everything correctly.")
      return False
  ...
  def dictify(self, json_data):
    try:
      ...
      if self.debug:
        print("ApiHandler: Error: "+str(e)+" Failed to dictify - check you've imported everything correctly.")
      return False
...
if __name__ == "__main__":
  ...
  if handler.verboseDebug:
    print("ApiHandler: ")
    print("---------------------------")
    print("Attempting connection and login: "+str(handler.login(authorization)))
    print("---------------------------")
  else:
  ...
  if handler.verboseDebug:
    ...
    print("Global token: "+str(handler.access_token)+" Local token: "+str(current_token))
    print("---------------------------")
  else: 
    current_token = handler.access_token
  ...
  if handler.verboseDebug:
    ...
    print("ApiHandler: ")
    print("---------------------------")
    print("Getting flags from people's beets in the passed list: "+str(handler.secret_flags))
    print("---------------------------")
  else:
    handler.get_flags(current_people)
